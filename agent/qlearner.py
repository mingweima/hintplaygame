...
class LSTM(nn.Module):
    ...
    def __init__(self, input_size, output_size, hidden_size=128, num_layers=1, n_hid=128):
        ...
        self.model = nn.Sequential(
            nn.Linear(hidden_size * hidden_size, n_hid),
            nn.ReLU(),
            nn.Linear(n_hid, n_hid),
            nn.ReLU(),
            nn.Linear(n_hid, output_size)
    ...
    def forward(self, x):
        ...
        out = output.reshape(-1, self.hidden_size * self.hidden_size)
...
class QLearner:
    def __init__(self, player, env, policy_type='ff', hp=hp_default):
        ...
        if policy_type == 'FF':
            ...
            self.policy_net = LSTM(card_dim, self.action_space_size, hidden_size=(1 + 2 * hp.hand_size))
            self.policy_net.to(device)
